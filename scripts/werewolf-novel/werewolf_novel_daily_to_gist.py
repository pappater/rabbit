#!/usr/bin/env python3
"""
Daily Gemini Werewolf Novel Generator
Generates one chapter per day using Google Gemini AI in a contemporary fantasy romance style,
...
WEREWOLF_GIST_ID = os.environ.get("WEREWOLF_GIST_ID")

DOCS_DIR = Path(__file__).parent.parent.parent / "docs" / "werewolf-novel"
PROMPTS_DIR = Path(__file__).parent / "prompts"

# Theme for the novel
THEME = "Ancient werewolf kingdoms, complete devotion, forbidden love, and the supernatural bond between fated mates"
...
def parse_chapter_names_from_outline(outline_content):
    ...
    for line in lines:
        # Look for lines like "### Chapter 1: The Raid"
        if line.strip().startswith('### Chapter '):
            try:
                # Extract the chapter number and name
                # Format: "### Chapter 1: The Raid"
...
def update_chapters_json(gist):
    ...
    for filename in chapter_files:
        try:
            ...
            chapter_data = {
                ...
                "filename": filename,
                "url": raw_url,
                "gist_url": f"https://gist.github.com/{WEREWOLF_GIST_ID}#{filename}"
            }
    ...
    chapters_data = {
        "novel_title": "Moonbound Devotion",
        "total_chapters": len(chapters),
        "last_updated": datetime.now().strftime("%Y-%m-%d %H:%M:%S UTC"),
        "gist_id": WEREWOLF_GIST_ID,
        "completed": False,
        "chapters": chapters
    }
...
def update_gist(chapter_num, chapter_text, continuity_log, gist):
    ...
    print(f"Updating Gist {WEREWOLF_GIST_ID}...")

    sanitized_files = _sanitize_gist_files(files)

    try:
        gist.edit(
            description=f"Moonbound Devotion - Werewolf Romance Novel - {chapter_num} Chapters",
    ...
    g = Github(GIST_TOKEN)
    refreshed_gist = g.get_gist(WEREWOLF_GIST_ID)
    ...
    print(f"  View at: https://gist.github.com/{WEREWOLF_GIST_ID}")
...
def ensure_first_chapter_in_gist(gist, series_bible, outline, summaries):
    ...
    summaries_content = summaries or "# Chapter Summaries: Moonbound Devotion\n"
    summaries_content += f"\n\n## Chapter 1\n\n{summary}"
    ...
    print(f"Publishing Chapter 1 to Gist {WEREWOLF_GIST_ID}...")

    sanitized_files = _sanitize_gist_files(files)

    try:
        gist.edit(
            description="Moonbound Devotion - Werewolf Romance Novel - 1 Chapter",
            files=sanitized_files
        )
    except AssertionError as exc:
        # Defensive fallback: convert any remaining malformed values into {'content': ...} and retry once
...
def main():
    ...
    if not WEREWOLF_GIST_ID:
        print("ERROR: WEREWOLF_GIST_ID environment variable not set")
        sys.exit(1)
    
    print("=" * 60)
    print("Daily Gemini Werewolf Romance Novel Generator")
    print("=" * 60)
    ...
    g = Github(GIST_TOKEN)
    gist = g.get_gist(WEREWOLF_GIST_ID)
    print(f"✓ Connected to Gist {WEREWOLF_GIST_ID}")
    ...
    # Update summary file
    summaries_content = load_file(DOCS_DIR / "summaries.md") or "# Chapter Summaries: Moonbound Devotion\n"
    summaries_content += f"\n\n## Chapter {chapter_num}\n\n{summary}"
    save_file(DOCS_DIR / "summaries.md", summaries_content)
