#!/usr/bin/env python3
"""
Daily Gemini Novel Generator - The Indifferent Shore
Generates one chapter per day using Google Gemini AI in a Camus-like voice,
...
GIST_ID = os.environ.get("STRANGER_GIST_ID")

DOCS_DIR = Path(__file__).parent.parent.parent / "docs" / "stranger-novel"
...
def parse_chapter_names_from_outline(outline_content):
    ...
    for line in lines:
        # Look for lines like "### Chapter 1: The Death"
        if line.strip().startswith('### Chapter '):
            try:
                # Extract the chapter number and name
                # Format: "### Chapter 1: The Death"
...
def update_chapters_json(gist):
    ...
    chapters_data = {
        "novel_title": "The Indifferent Shore",
...
def update_gist(chapter_num, chapter_text, continuity_log, gist):
    ...
    try:
        gist.edit(
            description=f"Daily Camus-style Novel - {chapter_num} Chapters",
...
def ensure_first_chapter_in_gist(gist, series_bible, outline, summaries):
    ...
    try:
        gist.edit(
            description="Daily Camus-style Novel - 1 Chapter",
            files=sanitized_files
        )
    except AssertionError as exc:
        # Defensive fallback: convert any remaining malformed values into {'content': ...} and retry once
...
def main():
    ...
    if not GIST_ID:
        print("ERROR: STRANGER_GIST_ID environment variable not set")
        sys.exit(1)
    
    print("=" * 60)
    print("Daily Gemini Novel Generator - The Indifferent Shore")
    print("=" * 60)
